...
BEGIN
	SET NOCOUNT ON;
	DECLARE @trialCount INT = 0;
	DECLARE @trialCounter INT = 0;
	DECLARE @szenarioTrials TABLE
	(
		trialId int, 
		szenarioTrialNumber int
	)
	DECLARE @currentTrialId int;
	DECLARE @startTime datetime2;
	DECLARE @endTime datetime2;
	DECLARE @trialSwitchReactionTimeMs TABLE
	(
		szenarioTrialNumber int, 
		trialSwitchReactionTimeMs int
	)

	INSERT INTO @szenarioTrials (trialId, szenarioTrialNumber)
	SELECT _trial.id, _szenario_trial_number.szenario_trial_number FROM ( _trial
				INNER JOIN _measure_file on _measure_file.id = _trial.measure_file_id
				INNER JOIN _study on _study.id = _trial.study_id
				INNER JOIN _group on _group.id = _trial.group_id
				INNER JOIN _szenario on _szenario.id = _trial.szenario_id
				...
				ORDER BY _szenario_trial_number.szenario_trial_number;;

	SELECT @trialCount = COUNT(trialId) FROM @szenarioTrials;

	WHILE @trialCounter < @trialCount
	BEGIN

		SELECT @currentTrialId = trialId FROM @szenarioTrials
			WHERE szenarioTrialNumber = (@trialCounter + 1);

		SELECT TOP 1 @startTime = _measure_data_filtered.time_stamp FROM _measure_data_filtered
					WHERE _measure_data_filtered.trial_id = @currentTrialId
		...
		SELECT TOP 1 @endTime = _measure_data_filtered.time_stamp FROM _measure_data_filtered
					WHERE _measure_data_filtered.trial_id = @currentTrialId AND _measure_data_filtered.position_status = 1
					ORDER BY _measure_data_filtered.time_stamp;

		INSERT INTO @trialSwitchReactionTimeMs (szenarioTrialNumber, trialSwitchReactionTimeMs)
			SELECT (@trialCounter + 1), DATEDIFF("MS",@startTime,@endTime);

		SET @trialCounter = @trialCounter + 1;
	END;

	SELECT * FROM @trialSwitchReactionTimeMs
	RETURN
END
...
CREATE procedure [dbo].[getSetDurationTimes]
	@studyName varchar(max),
	@groupName varchar(max),
	@szenarioName varchar(max),
	@subjectID int,
...
BEGIN
	...
	DECLARE @trialsPerSet INT = 16;
	DECLARE @setCount INT = 0;
	DECLARE @setCounter INT = 0;
	DECLARE @szenarioTrials TABLE
	(
		trialId int, 
		szenarioTrialNumber int
	...
	DECLARE @startTrialId INT;
	DECLARE @endTrialId INT;
	DECLARE @startTime datetime2;
	DECLARE @endTime datetime2;

	DECLARE @setDurationTimeMs TABLE
	(
		szenarioTrialNumber int, 
		setDurationTimeMs int
	)
	...
	SELECT @setCount = COUNT(trialId) / @trialsPerSet FROM @szenarioTrials;

	WHILE @setCounter < @setCount
	BEGIN
		SELECT @startTrialId = trialId FROM @szenarioTrials
		WHERE szenarioTrialNumber = ((@setCounter * @trialsPerSet) + 1);

		SELECT @endTrialId = trialId FROM @szenarioTrials
		WHERE szenarioTrialNumber = ((@setCounter + 1)*@trialsPerSet);

		SELECT TOP 1 @startTime = _measure_data_filtered.time_stamp FROM _measure_data_filtered
				WHERE _measure_data_filtered.trial_id = @startTrialId
				ORDER BY _measure_data_filtered.time_stamp;

		SELECT TOP 1 @endTime = _measure_data_filtered.time_stamp FROM _measure_data_filtered
				WHERE _measure_data_filtered.trial_id = @endTrialId
				ORDER BY _measure_data_filtered.time_stamp DESC;

		INSERT INTO @setDurationTimeMs (szenarioTrialNumber, setDurationTimeMs)
		SELECT (@setCounter + 1), DATEDIFF("MS",@startTime,@endTime);

	   SET @setCounter = @setCounter + 1;
	END;

	SELECT * FROM @setDurationTimeMs
	RETURN
END
