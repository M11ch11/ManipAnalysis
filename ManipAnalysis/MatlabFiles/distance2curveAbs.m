...
function distance = distance2curveAbs(trajectory, targetNumber)
...
for i=1:nTraj
    ...
    if(rotXvalues(i) < 0)               % if trial point is out of range (behind origin)
        dist(i) = sqrt((rotXvalues(i))^2 + (rotZvalues(i))^2);              % distance to origin using pythagorean law
    elseif(rotXvalues(i) > baseLength)  % if trial point is out of range (behind target)
        dist(i) = sqrt((rotXvalues(i)-baseLength)^2 + (rotZvalues(i))^2);   % distance to target point using pythagorean law after translation in direction of origin of 0.1m
    else
        dist(i) = abs(rotZvalues(i));
    end
