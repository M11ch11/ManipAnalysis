...
{
    internal class T1_ccw_01_Fam_Base_TrainA : ISzenarioDefinition
    {
        public const string StudyName = "Study 8";

        public const string SzenarioName = "T1_ccw_01_Fam_Base_TrainA";
        ...
        public override Trial setTrialMetadata(ManipAnalysisGui myManipAnalysisGui, Trial trial)
        {
            ...
            else
            {
                ...
                {
                    myManipAnalysisGui.WriteToLogBox("Correcting TrialNumberInSzenario: " + trial.TrialNumberInSzenario + " => " + (trial.TrialNumberInSzenario - 3) + ", Target " + trial.Target.Number);
                    trial.TrialNumberInSzenario -= 3;
                }
                else if (trial.TrialNumberInSzenario > 179)
                {
                    myManipAnalysisGui.WriteToLogBox("Correcting TrialNumberInSzenario: " + trial.TrialNumberInSzenario + " => " + (trial.TrialNumberInSzenario - 7) + ", Target " + trial.Target.Number);
                ...
                else if ((trial.Target.Number >= 24 && trial.Target.Number <= 26) || (trial.Target.Number >= 34 && trial.Target.Number <= 36)) // ErrorClampTrial + CCW ForceField
                {
                ...
                else if (trial.Target.Number >= 81 && trial.Target.Number <= 83) // 30s Pause + Wechsel R=>L + CW ForceField
                {
